...
マーカー文言を変更する場合は `scripts/watch-pr.sh` と `scripts/check-pr.sh` の `auto_fix_marker` も同時に更新すること。

## 例外: bot / CI が自動投稿する持続的メタコメントは別カテゴリで除外

「指摘内容を含むコメント」ではなく、bot や CI が PR ライフサイクルに紐づけて自動投稿する持続的メタコメント (人手レビューではなく対応するアクションも存在しない) は、別カテゴリのマーカー集合として除外する。`watch-pr.sh` / `check-pr.sh` の `persistent_meta_markers_json` 変数 (JSON 配列) で管理する。

デフォルトは空。プロジェクトに issue トラッカーのリンクバックや CI の進捗通知などの bot コメントがある場合、その本文に含まれる HTML コメントマーカーをこの配列に追加する。
...
- PR トップレベルコメント / レビュー本体: 本文にマーカーを含むものを除外してカウント (resolve の概念がないため)
