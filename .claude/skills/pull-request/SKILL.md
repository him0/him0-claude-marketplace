...
2. PR を作成する (`--base <branch>` 指定時は `gh pr create --base <branch>`。未指定時はデフォルトブランチ main が base になる)
3. このリポジトリに PR テンプレートはない。タイトルは Conventional Commits 形式 (例: `feat(git-ops-kit)!: ...`)、本文は日本語で `## 概要` / `## 変更内容` の構成で書き、動作確認した内容があれば `## 検証` を加える
4. `--base` で先行 PR のブランチを指定した場合 (スタック PR) は、PR description の冒頭に「Stacked on #<先行PR番号>」を明記する
5. <Ticket-URL> が指定された場合は、`## 概要` に参照リンクとして含める (issue を閉じる PR なら auto-close キーワードを使う)
