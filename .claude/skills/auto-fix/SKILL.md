...
- このリポジトリの CI は Claude Code Review (`claude-code-review.yml`) と @claude 応答 (`claude.yml`) の 2 workflow のみで、test / lint / build は存在しない。CI 失敗はほぼ workflow 実行エラー (トークン未設定等のインフラ起因) であり、ローカルでのテスト再実行は該当しない。Claude review の指摘はレビューコメントとして Step 3 で対応する
