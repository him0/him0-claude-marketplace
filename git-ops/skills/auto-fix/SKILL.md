...
このコマンドは「未解消の指摘を follow-up に倒した上で直接 approve する」明示オーバーライドであり、**人間の判断が必要な場面で承認がかかってしまうリスクがある**。auto-fix のフローからは絶対に投稿しない。

- `gh pr comment` 等でこの文字列を含むコメントを自動投稿してはならない
- レビュー指摘の対応が難しい / watch ループから抜けられない場合の逃げ道として使わない
